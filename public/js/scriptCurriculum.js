document.addEventListener("DOMContentLoaded", () => {
    ...
    */
    ...
    const inicio = () => {
        fetch('../info-columns/curriculumCols.json')
        .then(response => {
            if(!response.ok) {
                throw new Error('Error al cargar el archivo curriculumCols.json')
            }
            return response.json();
        }). then(data => {
            generarInfoCols(data);
        }). catch(err => {
            console.error('Hubo un problema al cargar los datos curriculum:', err);
        })
    }
